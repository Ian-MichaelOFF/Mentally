...
// Configuración de Multer para manejar la carga de imágenes (para registro de maestros)
...
// Middleware para verificar sesión de maestro
const verificarSesionMaestro = (req, res, next) => {
...
// Configuración de multer para perfil de maestros - SIMPLIFICADA
const storageMaestro = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, 'uploads', 'maestros');
...
const uploadMaestro = multer({
  storage: storageMaestro,
...
// Ruta para guardar alumnos - MODIFICADA PARA INCLUIR FECHA DE NACIMIENTO
app.post("/guardar", (req, res) => {
  console.log("Datos recibidos:", req.body);
  const { nombre, apellido, Usuario, contraseña, Pregunta_seguridad, Respuesta, fechaNacimiento } = req.body;

  // Validar que todos los campos estén presentes
  if (!nombre || !apellido || !Usuario || !contraseña || !Pregunta_seguridad || !Respuesta || !fechaNacimiento) {
    return res.status(400).json({
      error: "Todos los campos son obligatorios",
      detalles: "Faltan campos por completar"
    });
  }

  const nuevoUsuario = 
    "INSERT INTO alumnos (nombre, apellido, Usuario, contraseña, Pregunta_seguridad, Respuesta, Fecha) VALUES (?, ?, ?, ?, ?, ?, ?)";

  pool.query(
    nuevoUsuario,
    [nombre, apellido, Usuario, contraseña, Pregunta_seguridad, Respuesta, fechaNacimiento],
    (err, result) => {
      if (err) {
...
// -----------------------------------------------------
// Ruta para iniciar sesión CORREGIDA (incluye nombre y apellido y Fecha)
// -----------------------------------------------------
app.post("/login", (req, res) => {
  ...
  // Consulta incluyendo fecha de nacimiento
  const query = "SELECT IDalumno, nombre, apellido, Usuario, Respuesta, Imagen, Fecha FROM alumnos WHERE Usuario = ? AND contraseña = ?";
  ...
  pool.query(query, [name, password], (err, results) => {
    ...
    req.session.user = {
      ...
      tipo: "alumno" // Para diferenciar de maestro
    };
...
// -----------------------------------------------------
// Ruta para verificar sesión de alumno
// -----------------------------------------------------
...
// -----------------------------------------------------
// Ruta para obtener datos del alumno - INCLUYE FECHA
// -----------------------------------------------------
app.get("/api/alumno", (req, res) => {
  ...
  // Consulta incluyendo fecha de nacimiento
  const query = "SELECT IDalumno, nombre, apellido, Usuario, Respuesta, Imagen, Fecha FROM alumnos WHERE Usuario = ?";
...
// -----------------------------------------------------
// Ruta para actualizar imagen de perfil Alumno
// -----------------------------------------------------
...
// Actualizar imagen de perfil del maestro (para imágenes de stock)
app.put("/api/maestro", (req, res) => {
  ...
  pool.query(query, [imagen, req.session.user.IDmaestro], (err, results) => {
    if (err) {
      console.error("Error al actualizar perfil:", err);
      return res.status(500).json({ message: "Error al actualizar perfil" });
    }
...
// Ruta para subir foto de perfil personalizada del maestro
app.post("/api/maestro/upload-foto", 
  verificarSesionMaestro,
  uploadMaestro.single('foto'),
  (req, res) => {
    if (!req.file) {
    ...
    const finalFilename = `maestro_${req.session.user.IDmaestro}_${Date.now()}${ext}`;
    ...
    console.log('Ruta temporal completa:', tempPath);
    console.log('Renombrando a:', finalFilename);
    ...
    const querySelect = "SELECT Imagen FROM maestros WHERE IDmaestro = ?";
    
    pool.query(querySelect, [req.session.user.IDmaestro], (err, results) => {
      if (err) {
        console.error("Error al consultar imagen anterior:", err);
      } else if (results.length > 0 && results[0].Imagen) {
        const imagenAnterior = results[0].Imagen;
        
        if (imagenAnterior.includes('uploads/maestros/')) {
 ...
 // Guardar ruta en BD (sin barra inicial)
    const rutaImagen = `uploads/maestros/${finalFilename}`;
    console.log('Ruta a guardar en BD:', rutaImagen);
    
    const queryUpdate = "UPDATE maestros SET Imagen = ? WHERE IDmaestro = ?";
    
    pool.query(queryUpdate, [rutaImagen, req.session.user.IDmaestro], (err, results) => {
      if (err) {
        console.error("Error al actualizar imagen en BD:", err);
        if (fs.existsSync(finalPath)) {
          fs.unlinkSync(finalPath);
        }
        return res.status(500).json({ message: "Error al actualizar imagen en la base de datos" });
      }
...
app.delete("/api/maestro/foto", (req, res) => {
  if (!req.session.user || req.session.user.tipo !== "maestro") {
    return res.status(401).json({ message: "No autorizado" });
  }
  ...
  pool.query(querySelect, [req.session.user.IDmaestro], (err, results) => {
    if (err) {
      console.error("Error al consultar imagen:", err);
      return res.status(500).json({ message: "Error en el servidor" });
    }

    if (results.length > 0 && results[0].Imagen && results[0].Imagen.includes('uploads/maestros/')) {
      const rutaImagen = path.join(__dirname, results[0].Imagen);
      if (fs.existsSync(rutaImagen)) {
        fs.unlinkSync(rutaImagen);
        console.log('Foto personalizada eliminada');
      }
    }

    const queryUpdate = "UPDATE maestros SET Imagen = ? WHERE IDmaestro = ?";
    pool.query(queryUpdate, ['default.png', req.session.user.IDmaestro], (err, results) => {
      if (err) {
