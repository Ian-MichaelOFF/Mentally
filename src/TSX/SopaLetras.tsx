...
import cerebrin from "/public/logos/CEREBRITIN.png"// Asegúrate de que la ruta sea correcta
...
const SopaDeLetras: React.FC = () => {
  ...
  const [currentSelection, setCurrentSelection] = useState<{row: number, col: number, letter: string}[]>([]);
  const [direction, setDirection] = useState<{dRow: number, dCol: number} | null>(null);
  ...
  const guardarPartida = async () => {
    try {
      ...
      const dificultad = dificultadMapping[currentLevel as keyof typeof dificultadMapping];
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        if (filledGrid[i][j] === '') {
          ...
          filledGrid[i][j] = String.fromCharCode(65 + Math.floor(Math.random() * 26));
        }
      }
    }
    ...
    while (!placed && attempts < 100) {
      ...
      if (direction === 'horizontal') {
        ...
        for (let i = 0; i < word.length; i++) {
          ...
          if (grid[startRow][startCol + i] !== '' && grid[startRow][startCol + i] !== word[i].toLowerCase()) {
            canPlace = false;
            break;
          }
        }
        ...
        for (let i = 0; i < word.length; i++) {
          // Comprobar si la celda está vacía o ya tiene la misma letra
          if (grid[startRow + i][startCol] !== '' && grid[startRow + i][startCol] !== word[i].toLowerCase()) {
            canPlace = false;
            break;
          }
        }
        ...
        for (let i = 0; i < word.length; i++) {
          // Comprobar si la celda está vacía o ya tiene la misma letra
          if (grid[startRow + i][startCol + i] !== '' && grid[startRow + i][startCol + i] !== word[i].toLowerCase()) {
            canPlace = false;
            break;
          }
        }
  ...
  const selectCell = (row: number, col: number, letter: string) => {
    ...
    if (currentSelection.length > 1 && direction) {
      ...
      if (row - last.row !== direction.dRow || col - last.col !== direction.dCol) {
        clearSelection();
        return;
      }
    }
    ...
    const wordToFind = selectedWords.find(word => word === formedWord || word === reversed);
  ...
  const showEndOptions = () => {
    ...
    const maxTime = currentLevel === 'easy' ? 60 : currentLevel === 'medium' ? 45 : 35;
    ...
    if (foundWordsCount === totalWords) {
      setMessage(`🎉 **¡Encontraste todas las palabras en ${formattedTime}!** tu puntaje fue: ${score}`);
    } else {
      setMessage(`🔍 Encontraste ${foundWordsCount} de ${totalWords} palabras. Te faltaron ${totalWords - foundWordsCount}. Tiempo transcurrido: ${formattedTime}`);
    }
  };
      ...
      <div id="game-container">
        ...
        {!gameStarted && (
          <div id="welcome-description">
            <p>
              👋 <strong>Bienvenido</strong>, elige un nivel de dificultad y encuentra
              todas las palabras escondidas en la sopa de letras.
            </p>
          </div>
        )}
        ...
        {gameStarted && !gameEnded && (
          <div id="timer">
            <p>Tiempo restante: <span id="time-left">{formatTimeDisplay(timeLeft)}</span></p>
          </div>
        )}
        ...
        {gameStarted && (
          <div id="words-to-find" style={{ display: gameStarted ? 'block' : 'none' }}>
        ...
        {gameEnded && (
          <div id="end-options">
            <button id="play-again-button" onClick={playAgain}>Jugar de nuevo</button>
